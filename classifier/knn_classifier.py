...
backbone_trt = torch2trt(backbone, [data_sample], use_onnx=True)
...
def train(train_dir, model_save_path=None, n_neighbors=None, knn_algo='ball_tree', verbose=False):
    ...
    for class_dir in os.listdir(train_dir):
        ...
        for img_path in os.listdir(os.path.join(train_dir, class_dir)):
            # image = Image.open(os.path.join(train_dir, class_dir,img_path))
            image = cv2.imread(os.path.join(train_dir, class_dir,img_path))
            # bb_and_features = extract_feature(image, backbone_trt)
            ...
            #     y.append(class_dir)
                # print(bb_and_feature[1].cpu().detach().numpy()[0][0].shape)
            feature = extract_feature_v2.extract_feature(img=image, backbone=backbone_trt)
            X.append(feature.cpu().detach().numpy()[0][0])
            y.append(class_dir)
...
def predict(img_path, knn_clf=None, model_path=None, distance_threshold=25):
    """
    ...
    X_img_path = Image.open(img_path)
    print(X_img_path)
    bb_and_features = extract_feature(X_img_path, backbone_trt)
    ...
    for bb_and_feature in bb_and_features:
        faces_encodings.append(bb_and_feature[1].cpu().detach().numpy()[0][0])
    faces_encodings = np.array(faces_encodings)
    # print(faces_encodings.shape)
    # Use the KNN model to find the best matches for the test face
    closest_distances = knn_clf.kneighbors(faces_encodings, n_neighbors=1)
    # print(closest_distances)
    are_matches = [closest_distances[0][i][0] <= distance_threshold for i in range(len(bb_and_features))]
...
if __name__ == "__main__":
    ...
    # classifier = train("data/images/aligned", model_save_path="classifier/trained_knn_model.clf", n_neighbors=99)
    ...
    predictions = predict("test/dung.jpg", model_path="classifier/trained_knn_model.clf")
    ...
    # show_prediction_labels_on_image("test/tuyen.jpg", predictions)